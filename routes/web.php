...
Route::post('/getCode', function(Request $request) {
    ...
    if($user) {
        $ts1 = strtotime($user->updated_at);
        $ts2 = strtotime(now()); 
        $seconds_diff = $ts2 - $ts1; 
         if (0.1 > $seconds_diff) {
           
            return response()->json([
                "message" => "time expire ".  $seconds_diff
                ],400);
...
Route::middleware(['IsInstalled', 'auth', 'SetSessionData', 'language', 'timezone'])->group(function () {
    ...
    Route::post('/orders/update-order-details/{id}', 'SellPosController@updateOrder')->name('orders.update_order_details');
    Route::get('/orders/get-order-details-product', 'SellPosController@getProduct')->name('orders.get_order_details_product');
    ...
    Route::get('/coupons', 'CouponController@index')->name("coupons.list");
